/*
id: int
formula: string 何らかの区切り文字で区切られた式
result: double 結果
createdDate: DateTime 計算した日
toList(): formulaを区切り文字に従って分離する関数
Map<String,dynamic> toJson(): JSON用のMapに変換
fromJson(Map<String,dynamic>)コンストラクタ: JSON用のMapから変換する。
全てのフィールドにfinalをつけてイミュータブルにする
*/

class ResultData {
  final int id;
  final String formula;
  final double result;
  final DateTime createdDate;

  //= new Resultdata(1,1+2,3,2022)みたいに作ったとき
  ResultData(this.id, this.formula, this.result, this.createdDate);

  //jsonはマップ型で、{id:int,formula:String,みたいな}
  //jonからresultDataを作るときに使う
  ResultData.fromJson(Map<String, dynamic> json)
      : id = json["id"],
        formula = json["formula"],
        result = json["result"],
        createdDate = json["createdDate"];

//フォーミュラが配列だったら、結合しておく = new ResultData(1,["1","+","2"],3,2022)みたいなとき
  ResultData.listFormula(
      this.id, List<String> listFormula, this.result, this.createdDate)
      : formula = listFormula.join(" ");

//ResultData型の何かをJsonのmapにする
  Map<String, dynamic> toJson() => {
        "id": id,
        "formula": formula,
        "result": result,
        "createdDate": createdDate,
      };
}
